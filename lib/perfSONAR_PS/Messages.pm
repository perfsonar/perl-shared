...
perfSONAR_PS::Messages

=head1 DESCRIPTION

A module that provides common methods for performing actions on message
constructs.  This module is a catch all for message related methods in the
...
sub startMessage {
    ...
    $attrs{"type"}         = $type;
    $attrs{"id"}           = $id;
    $attrs{"messageIdRef"} = $messageIdRef if defined $messageIdRef and $messageIdRef;
...
sub startData {
    ...
    if ( not defined $id or $id eq q{} or not defined $metadataIdRef or $metadataIdRef eq q{} ) {
...
sub getResultCodeMessage {
    ...
    $n = startMessage( $output, $id, $messageIdRef, $type, q{}, undef );
    return $n if ( $n != 0 );
...
sub getResultCodeMetadata {
    ...
    if ( not defined $id or $id eq q{} or not defined $event or $event eq q{} ) {
        $logger->error( "Missing argument(s)." );
        return -1;
    }
...
sub getResultCodeData {
    ...
    if ( not defined $id or $id eq q{} or not defined $metadataIdRef or $metadataIdRef eq q{} or not defined $description or $description eq q{} ) {
        return -1;
    }
