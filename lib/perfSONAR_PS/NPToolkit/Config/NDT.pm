package perfSONAR_PS::NPToolkit::Config::NDT;
...
perfSONAR_PS::NPToolkit::Config::NDT

=head1 DESCRIPTION

Module for configuring the displayed NDT index page. Longer term, This should
...
currently in place. This uses a template file (tcpbw100_html.tmpl) to create
the new tcpbw100.html.
...
use fields 'TCPBW100_HTML_FILE', 'TCPBW100_HTML_TEMPLATE', 'ADMINISTRATOR_NAME', 'ADMINISTRATOR_EMAIL', 'ORGANIZATION_NAME', 'LOCATION';
...
the tcpbw100_html_file should point to the file that gets written by the
module, and tcpbw100_html_template should point to the template used to
generate the tcpbw100.html file.
...
sub init {
    ...
    my $parameters = validate(
        @params,
        {
            tcpbw100_html_file     => 0,
            tcpbw100_html_template => 0,
        }
    );

    # Initialize the defaults
    $self->{TCPBW100_HTML_FILE}     = $defaults{tcpbw100_html_file};
    $self->{TCPBW100_HTML_TEMPLATE} = $defaults{tcpbw100_html_template};

    # Override any
    $self->{TCPBW100_HTML_FILE}     = $parameters->{tcpbw100_html_file}     if ( $parameters->{tcpbw100_html_file} );
    $self->{TCPBW100_HTML_TEMPLATE} = $parameters->{tcpbw100_html_template} if ( $parameters->{tcpbw100_html_template} );
...
=head2 save({ restart_services => 0 })
    Saves the configuration to disk. NDT will simply read in the new file so
...
sub save {
    ...
    if ( $parameters->{restart_services} ) {
        ...
        # This current just updates the index.html so there's no need to
        # actually restart NDT.

        #        $res = restart_service({ name => "ndt" });
        #        if ($res == -1) {
        #            return -1;
        #        }
    }
...
sub last_modified {
    ...
    my ($mtime) = (stat ( $self->{TCPBW100_HTML_FILE} ) )[9];
...
=head2 generate_tcpbw100_html_file({})
Takes the configured values and the configured template and generates a string
containing the contents of the tcpbw100.html file.
=cut

sub generate_tcpbw100_html_file {
    my ( $self, @params ) = @_;
    my $parameters = validate( @params, {} );
    ...
    my $template_dir  = dirname( $self->{TCPBW100_HTML_TEMPLATE} );
    my $template_file = basename( $self->{TCPBW100_HTML_TEMPLATE} );
...
sub save_state {
    ...
    my %state = (
        tcpbw100_html_file     => $self->{TCPBW100_HTML_FILE},
        tcpbw100_html_template => $self->{TCPBW100_HTML_TEMPLATE},
...
sub restore_state {
    ...
    $self->{TCPBW100_HTML_FILE}     = $state->{tcpbw100_html_file};
    $self->{TCPBW100_HTML_TEMPLATE} = $state->{tcpbw100_html_template};
