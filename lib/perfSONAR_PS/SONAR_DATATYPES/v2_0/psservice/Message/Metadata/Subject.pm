package  perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject;
...
perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject  -  this is data binding class for  'subject'  element from the XML schema namespace psservice

=head1 DESCRIPTION

Object representation of the subject element of the psservice XML namespace.
Object fields are:
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject->new({xml => $xml_string});
...
use fields qw(nsmap idmap LOGGER id service  text );
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject';
        ...
        $subject = getElement({name =>   $LOCALNAME, 
			      ...
			      ns  =>    \@nss,
                              attributes => [
                                            ...
                                            'text' => (!($self->get_service)?$self->get_text:undef),
...
sub fromDOM {
    ...
    $self->set_text($dom->textContent) if(!($self->get_service) && $dom->textContent);
	...
	my $element;
        ...
        if ($tagname eq  'service' && $nsid eq 'psservice' && $self->can("get_$tagname")) {
                eval {
