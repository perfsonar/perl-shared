package  perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst;
...
perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst  -  this is data binding class for  'dst'  element from the XML schema namespace nmwgt

=head1 DESCRIPTION

Object representation of the dst element of the nmwgt XML namespace.
Object fields are:
...
The constructor accepts only single parameter, it could be a hashref with keyd  parameters hash  or DOM of the  'dst' element
...
=head1 SYNOPSIS
          ...
          use perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst;
          use Log::Log4perl qw(:easy);

          Log::Log4perl->easy_init();

          my $el =  perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst->new({xml => $xml_string});
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst';
...
sub  querySQL {
    my ($self, $query) = @_;
     ...
     $query->{metaData}{ip_name_dst}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{metaData}{ip_name_dst}) || ref($query->{metaData}{ip_name_dst});
     $query->{host}{ip_name}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{host}{ip_name}) || ref($query->{host}{ip_name});
     $query->{host}{ip_number}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{host}{ip_number}) || ref($query->{host}{ip_number});
     $query->{metaData}{ip_name_dst}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{metaData}{ip_name_dst}) || ref($query->{metaData}{ip_name_dst});
     $query->{host}{ip_name}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{host}{ip_name}) || ref($query->{host}{ip_name});
     $query->{host}{ip_number}= [ 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst' ] if!(defined $query->{host}{ip_number}) || ref($query->{host}{ip_number});
            ...
            foreach my $entry (@{$defined_table{$table}}) {
                if(ref($query->{$table}{$entry}) eq 'ARRAY') {
                    foreach my $classes (@{$query->{$table}{$entry}}) {
                         if($classes && $classes eq 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst') {
        
                            if    ($self->get_value && ( (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name_dst')) || (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->get_type eq 'ipv4')  && $entry eq 'ip_number')) )) {
                                $query->{$table}{$entry} =  $self->get_value;
                                $self->get_LOGGER->debug(" Got value for SQL query $table.$entry: " . $self->get_value);
                                last;  
                            }

                            elsif ($self->get_text && ( (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name_dst')) || (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->get_type eq 'ipv4')  && $entry eq 'ip_number')) )) {
...
=head2  asString()
 ...
 returns nicely formatted XML string  representation of the  dst object
