package  perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters;
...
perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters  -  this is data binding class for  'parameters'  element from the XML schema namespace histogram

=head1 DESCRIPTION

Object representation of the parameters element of the histogram XML namespace.
Object fields are:
...
=head1 SYNOPSIS
          ...
          use perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters;
          use Log::Log4perl qw(:easy);

          Log::Log4perl->easy_init();

          my $el =  perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters->new({xml => $xml_string});
...
use perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwg::Message::Parameters::Parameter;
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::SONAR_DATATYPES::v2_0::histogram::Message::Parameters';
...
sub new {
    ...
    $self->get_nsmap->mapname($LOCALNAME, 'histogram');
...
sub addParameter {
    ...
    $self->get_parameter && ref($self->get_parameter) eq 'ARRAY'?push @{$self->get_parameter}, $new:
                                                                 $self->set_parameter([$new]);
...
sub removeParameterById {
    my ($self, $id) = @_;
    if(ref($self->get_parameter) eq 'ARRAY' && $self->get_idmap->{parameter} &&  
       exists $self->get_idmap->{parameter}{$id}) {
...
sub fromDOM {
	...
	my $element;
        ...
        if ($tagname eq  'parameter' && $nsid eq 'nmwg' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwg::Message::Parameters::Parameter->new($childnode)
                };
