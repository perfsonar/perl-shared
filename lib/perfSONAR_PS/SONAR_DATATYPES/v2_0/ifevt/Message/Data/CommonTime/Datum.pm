package  perfSONAR_PS::SONAR_DATATYPES::v2_0::ifevt::Message::Data::CommonTime::Datum;
...
perfSONAR_PS::SONAR_DATATYPES::v2_0::ifevt::Message::Data::CommonTime::Datum  -  this is data binding class for  'datum'  element from the XML schema namespace ifevt
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::SONAR_DATATYPES::v2_0::ifevt::Message::Data::CommonTime::Datum->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::SONAR_DATATYPES::v2_0::ifevt::Message::Data::CommonTime::Datum->new({xml => $xml_string});
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::SONAR_DATATYPES::v2_0::ifevt::Message::Data::CommonTime::Datum';
...
sub getDOM {
    ...
    foreach my $textnode (qw/stateAdmin stateOper/) {
        if($self->{$textnode}) {
           ...
           $domtext?$datum->appendChild($domtext):
	             $self->get_LOGGER->logdie("Failed to append new text element $textnode to datum");
        }
    }
...
sub fromDOM {
	...
	my $element;
        ...
        if (!($self->get_timeValue && $self->get_timeType) && $tagname eq  'time' && $nsid eq 'nmtm' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmtm::Message::Data::CommonTime::Datum::Time->new($childnode)
                };
