package  perfSONAR_PS::SONAR_DATATYPES::v2_0::perfsonar::Message::Metadata::Subject;
...
perfSONAR_PS::SONAR_DATATYPES::v2_0::perfsonar::Message::Metadata::Subject  -  this is data binding class for  'subject'  element from the XML schema namespace perfsonar

=head1 DESCRIPTION

Object representation of the subject element of the perfsonar XML namespace.
Object fields are:
    ...
    Object reference:   service => type HASH,
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::SONAR_DATATYPES::v2_0::perfsonar::Message::Metadata::Subject->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::SONAR_DATATYPES::v2_0::perfsonar::Message::Metadata::Subject->new({xml => $xml_string});
...
use perfSONAR_PS::SONAR_DATATYPES::v2_0::nmtl3::Message::Metadata::Subject::Interface;
use perfSONAR_PS::SONAR_DATATYPES::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair;
use perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair;
use perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject::Service;
use fields qw(nsmap idmap LOGGER metadataIdRef id interface endPointPair service );
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::SONAR_DATATYPES::v2_0::perfsonar::Message::Metadata::Subject';
...
sub new {
    ...
    $self->get_nsmap->mapname($LOCALNAME, 'perfsonar');
...
sub getDOM {
    ...
    if($self->get_interface && blessed $self->get_interface && $self->get_interface->can("getDOM")) {
        my $interfaceDOM = $self->get_interface->getDOM($subject);
        $interfaceDOM?$subject->appendChild($interfaceDOM):$self->get_LOGGER->logdie("Failed to append  interface element with value:" .  $interfaceDOM->toString);
    }


    if(!($self->get_interface) && $self->get_endPointPair && blessed $self->get_endPointPair && $self->get_endPointPair->can("getDOM")) {
        my $endPointPairDOM = $self->get_endPointPair->getDOM($subject);
        $endPointPairDOM?$subject->appendChild($endPointPairDOM):$self->get_LOGGER->logdie("Failed to append  endPointPair element with value:" .  $endPointPairDOM->toString);
    }


    if($self->get_service && blessed $self->get_service && $self->get_service->can("getDOM")) {
        my $serviceDOM = $self->get_service->getDOM($subject);
        $serviceDOM?$subject->appendChild($serviceDOM):$self->get_LOGGER->logdie("Failed to append  service element with value:" .  $serviceDOM->toString);
    }
...
=head2 get_endPointPair

 accessor  for endPointPair, assumes hash based class

=cut

sub get_endPointPair {
    my($self) = @_;
    return $self->{endPointPair};
}

=head2 set_endPointPair

mutator for endPointPair, assumes hash based class
...
=head2 get_service
 ...
 accessor  for service, assumes hash based class
...
mutator for service, assumes hash based class
...
sub addEndPointPair {
    ...
    $self->get_endPointPair && ref($self->get_endPointPair) eq 'ARRAY'?push @{$self->get_endPointPair}, $new:$self->set_endPointPair([$new]);
    $self->get_LOGGER->debug("Added new to endPointPair");
    $self->buildIdMap; ## rebuild index map
    return $self->get_endPointPair;
}

=head2  removeEndPointPairById()

 removes specific element from the array of endPointPair elements by id ( if id is supported by this element )
...
sub removeEndPointPairById {
    my ($self, $id) = @_;
    if(ref($self->get_endPointPair) eq 'ARRAY' && $self->get_idmap->{endPointPair} &&  exists $self->get_idmap->{endPointPair}{$id}) {
        undef $self->get_endPointPair->[$self->get_idmap->{endPointPair}{$id}];
        my @tmp =  grep { defined $_ } @{$self->get_endPointPair};
        $self->set_endPointPair([@tmp]);
        $self->buildIdMap; ## rebuild index map
        return $id;
    } elsif(!ref($self->get_endPointPair)  || ref($self->get_endPointPair) ne 'ARRAY')  {
        $self->get_LOGGER->warn("Failed to remove  element because endPointPair not an array for non-existent id:$id");
    } else {
...
sub getEndPointPairById {
    ...
    if(ref($self->get_endPointPair) eq 'ARRAY' && $self->get_idmap->{endPointPair} && exists $self->get_idmap->{endPointPair}{$id} ) {
        return $self->get_endPointPair->[$self->get_idmap->{endPointPair}{$id}];
    } elsif(!ref($self->get_endPointPair) || ref($self->get_endPointPair) ne 'ARRAY')  {
        return $self->get_endPointPair;
    }
...
sub  querySQL {
    ...
    foreach my $subname (qw/interface endPointPair service/) {
        if($self->{$subname} && (ref($self->{$subname}) eq 'ARRAY' ||  blessed $self->{$subname})) {
            ...
            foreach my $el (@array) {
                if(blessed $el && $el->can('querySQL'))  {
                    $el->querySQL($query);
                    $self->get_LOGGER->debug("Querying subject  for subclass $subname");
                } else {
                    $self->get_LOGGER->logdie("Failed for subject Unblessed member or querySQL is not implemented by subclass $subname");
                }
           }
        }
    }
...
sub  buildIdMap {
    ...
    foreach my $field (qw/interface endPointPair service/) {
...
sub fromDOM {
	...
	my $element;
        ...
        if ($tagname eq  'interface' && $nsid eq 'nmtl3' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmtl3::Message::Metadata::Subject::Interface->new($childnode)
                };
        ...
        elsif (!($self->get_interface) && $tagname eq  'endPointPair' && $nsid eq 'nmwgt' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
                    $self->get_LOGGER->logdie(" Failed to load and add  EndPointPair : " . $dom->toString . " error: " . $EVAL_ERROR);
                     return;
                }
              $self->set_endPointPair($element); ### add another endPointPair  
            } 
        elsif (!($self->get_interface) && $tagname eq  'endPointPair' && $nsid eq 'nmtl4' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::nmtl4::Message::Metadata::Subject::EndPointPair->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
                    $self->get_LOGGER->logdie(" Failed to load and add  EndPointPair : " . $dom->toString . " error: " . $EVAL_ERROR);
                     return;
        ...
        elsif ($tagname eq  'service' && $nsid eq 'psservice' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::SONAR_DATATYPES::v2_0::psservice::Message::Metadata::Subject::Service->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
                    $self->get_LOGGER->logdie(" Failed to load and add  Service : " . $dom->toString . " error: " . $EVAL_ERROR);
                     return;
                }
              $self->set_service($element); ### add another service  
            } 
    }
    $self->buildIdMap;
