...
package perfSONAR_PS::PINGER_DATATYPES::PingER;
{
=head1 NAME

 perfSONAR_PS::PINGER_DATATYPES::PingER  -  this is a pinger message  handler object


=head1 DESCRIPTION

   it inherits everything from perfSONAR_PS::PINGER_DATATYPES::Message and only implements  request handlers with pinger specifics 
    
=head1 SYNOPSIS
             
  
	     use perfSONAR_PS::PINGER_DATATYPES::PingER ;
	   
	     
	     my ($DOM) = $requestMessage->getElementsByTag('message');
	    
	     my $message = new perfSONAR_PS::PINGER_DATATYPES::PingER($DOM);
             $message = new perfSONAR_PS::PINGER_DATATYPES::PingER({id => '2345', 
	                                                     type = 'SetupdataResponse',
							     metadata => {'id1' =>   <obj>},
							     data=> {'id1' => <obj>}}); 
	 
	    #######   add data element, namespaces will be added from this object to Message object namespace declaration
             $message->addPartById('id1', 'data', new perfSONAR_PS::PINGER_DATATYPES::PingER::data({id=> 'id1', metadataIdRef => 'metaid1' }));
        
	    ########add metadata element, namespaces will be added from this object to Message object namespace declaration
	     $message->addPartById('id1', 'metadata',  new perfSONAR_PS::PINGER_DATATYPES::PingER::metadata({id=> 'id1' });
...
use perfSONAR_PS::PINGER_DATATYPES::v2_0::Element;
use perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message; 
use perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data;
use perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata;
...
use perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data::CommonTime;

use perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwgr::Message::Data::Datum;
...
use perfSONAR_PS::PINGER_DATATYPES::v2_0::pinger::Message::Data::CommonTime::Datum;
...
use base qw(perfSONAR_PS::PINGER_DATATYPES::Message);
 
use constant  CLASSPATH =>  'perfSONAR_PS::PINGER_DATATYPES::v2_0::PingER';
...
sub  MetadataKeyRequest {
    ...
    ##my $message_limit = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Parameters::Parameter->new({name => 'sizeLimit', value =>  $_sizeLimit});
	...
	foreach my  $metaid (keys %{$objects}) {
	    ...
	    my $key =   perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data::Key->new({id => $metaid });
	    $logger->debug(" KEY  created: \n " .  $key->asString );
	    my $data  = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data->new({ id => "data" . $response->dataID, metadataIdRef => "meta$md_id" , key => $key  });
...
sub  SetupDataRequest  {
    ...
    ##my $message_params = perfSONAR_PS::PINGER_DATATYPES::v2_0::select::Message::Metadata::Parameters->new();
    ##my $message_limit  = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Parameters::Parameter->new({name => 'sizeLimit', value =>  $_sizeLimit});
    ##$message_params->addParameter($message_limit);
    ##$response->addParameters($message_params);
    #### setting status URI for response
     ...
     if($requestmd->key  && $requestmd->key->id) {      
	    ...
	    ############################################################   here add all those found data elements
	        ...
	        if(%{$objects_hashref->{$metaid}}) { 
	            my $data  = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data->new({id => "data". $response->dataID, metadataIdRef => "meta" . $metaids->{$metaid}});
	            foreach my $data_metaid (sort { $a <=> $b} keys %{$objects_hashref->{$metaid}}) {   
		          ...
		          my $ctime = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data::CommonTime->new({ type => 'unix', value => $timev});
		          ...
		          foreach my $entry (qw/minRtt maxRtt medianRtt meanRtt clp iqrIpd lostPercent  maxIpd minIpd meanIpd duplicates outOfOrder/) {
		              ...
		              my $datum = perfSONAR_PS::PINGER_DATATYPES::v2_0::pinger::Message::Data::CommonTime::Datum->new({name => $entry , value  =>  $value});
...
sub  ressurectMd {
    ...
    $md  = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata->new(); 
    my $key =  perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Key->new({id => $metaid });
   
    my $subject = perfSONAR_PS::PINGER_DATATYPES::v2_0::pinger::Message::Metadata::Subject->new({ id => "subj$metaid",
  		      endPointPair => perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair->new({		      
  		                          src => perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->new({ value =>  $params->{md_row}->{ip_name_src}, type => 'hostname'}),
     				          dst => perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Dst->new({ value =>  $params->{md_row}->{ip_name_dst}, type => 'hostname'})
     				      }) 
     		  });		  
     
    my $pinger_params =   perfSONAR_PS::PINGER_DATATYPES::v2_0::pinger::Message::Metadata::Parameters->new({ id => "params$metaid" });
    my $param_arrref = []; 
    no strict 'refs';
      
    foreach my $pparam (qw/count packetSize ttl transport packetInterval protocol/) { 
        if($params->{md_row}->{$pparam}) {
  	   push @{$param_arrref} , perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Parameters::Parameter->new({name =>  $pparam , value => $params->{md_row}->{$pparam}});
        }
    }
    use strict;
    $md->subject($subject); 
...
sub _createTimeSelect {
    ...
    if($params->{timequery}->{eq}) {
        push @{$selects} , perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Parameters::Parameter->new({name => 'timeStamp', value =>  $params->{timequery}->{eq}});
    } elsif($params->{timequery}->{gt} && $params->{timequery}->{lt}) {
        push @{$selects} , perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Parameters::Parameter->new({name => 'startTime', value =>  $params->{timequery}->{gt}});
        push @{$selects} , perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata::Parameters::Parameter->new({name => 'endTime', value =>  $params->{timequery}->{lt}});
    }  
    if($selects  && @{$selects}) {
        my $md =   perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata->new();
	$md->subject(perfSONAR_PS::PINGER_DATATYPES::v2_0::select::Message::Metadata::Subject->new({id => "subj" .  $params->{response}->mdID}));
	
        my $param_selects = perfSONAR_PS::PINGER_DATATYPES::v2_0::select::Message::Metadata::Parameters->new({ id => "params".$params->{response}->mdID,   parameter => $selects});
