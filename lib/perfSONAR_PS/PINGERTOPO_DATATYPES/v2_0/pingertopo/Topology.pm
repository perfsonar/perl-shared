package  perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology;
...
perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology  -  this is data binding class for  'topology'  element from the XML schema namespace pingertopo

=head1 DESCRIPTION

Object representation of the topology element of the pingertopo XML namespace.
Object fields are:
...
The constructor accepts only single parameter, it could be a hashref with keyd  parameters hash  or DOM of the  'topology' element
...
=head1 SYNOPSIS
          ...
          use perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology;
          use Log::Log4perl qw(:easy);

          Log::Log4perl->easy_init();

          my $el =  perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology->new({xml => $xml_string});
...
use fields qw(nsmap idmap LOGGER  domain );
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology';
...
sub getDOM {
    ...
    eval { 
        ...
        $topology = getElement({name =>   $LOCALNAME, 
	                      parent => $parent,
			      ns  =>    \@nss,
                              attributes => [
    ...
    if($self->get_domain && ref($self->get_domain) eq 'ARRAY') {
        foreach my $subel (@{$self->get_domain}) {
            if(blessed $subel && $subel->can("getDOM")) {
                my $subDOM = $subel->getDOM($topology);
                $subDOM?$topology->appendChild($subDOM):$self->get_LOGGER->logdie("Failed to append  domain element  with value:" .  $subDOM->toString);
            }
        }
    }
...
=head2 get_domain
 ...
 accessor  for domain, assumes hash based class
...
mutator for domain, assumes hash based class
...
sub addDomain {
    ...
    $self->get_domain && ref($self->get_domain) eq 'ARRAY'?push @{$self->get_domain}, $new:
                                                                 $self->set_domain([$new]);
    $self->get_LOGGER->debug("Added new to domain");
...
=head2  removeDomainById()
 ...
 removes specific element from the array of domain elements by id ( if id is supported by this element )
...
sub removeDomainById {
    my ($self, $id) = @_;
    if(ref($self->get_domain) eq 'ARRAY' && $self->get_idmap->{domain} &&  
       exists $self->get_idmap->{domain}{$id}) {
        undef $self->get_domain->[$self->get_idmap->{domain}{$id}];
        my @tmp =  grep { defined $_ } @{$self->get_domain};
        $self->set_domain([@tmp]);
        $self->buildIdMap; ## rebuild index map
        return $id;
    } elsif(!ref($self->get_domain)  || ref($self->get_domain) ne 'ARRAY')  {
        $self->get_LOGGER->warn("Failed to remove  element because domain not an array for non-existent id:$id");
    } else {
...
sub getDomainById {
    ...
    if(ref($self->get_domain) eq 'ARRAY' && $self->get_idmap->{domain} && 
       exists $self->get_idmap->{domain}{$id} ) {
        return $self->get_domain->[$self->get_idmap->{domain}{$id}];
    } elsif(!ref($self->get_domain) || ref($self->get_domain) ne 'ARRAY')  {
        return $self->get_domain;
    }
        ...
        if($self->{$subname} && (ref($self->{$subname}) eq 'ARRAY' ||  blessed $self->{$subname})) {
            ...
            foreach my $el (@array) {
                if(blessed $el && $el->can('querySQL'))  {
                    $el->querySQL($query);
                    $self->get_LOGGER->debug("Querying topology  for subclass $subname");
                } else {
                    $self->get_LOGGER->logdie("Failed for topology Unblessed member or querySQL is not implemented by subclass $subname");
                }
           }
        }
    }
...
=head2  asString()
 ...
 returns nicely formatted XML string  representation of the  topology object
...
sub fromDOM {
	...
	my $element;
        ...
        if ($tagname eq  'domain' && $nsid eq 'pingertopo' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::pingertopo::Topology::Domain->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
                    $self->get_LOGGER->logdie(" Failed to load and add  Domain : " . $dom->toString . " error: " . $EVAL_ERROR);
                     return;
                }
               ($self->get_domain && ref($self->get_domain) eq 'ARRAY')?push @{$self->get_domain}, $element:
                                                                                                        $self->set_domain([$element]);; ### add another domain  
            } 
    }
    $self->buildIdMap;
