package  perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port;
...
perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port  -  this is data binding class for  'port'  element from the XML schema namespace nmtl3

=head1 DESCRIPTION

Object representation of the port element of the nmtl3 XML namespace.
Object fields are:
...
The constructor accepts only single parameter, it could be a hashref with keyd  parameters hash  or DOM of the  'port' element
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port->new({xml => $xml_string});
...
use perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port::IpAddress;
use fields qw(nsmap idmap LOGGER id ipAddress );
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port';
...
sub getDOM {
    ...
    eval { 
        ...
        $port = getElement({name =>   $LOCALNAME, 
	                      parent => $parent,
			      ns  =>    \@nss,
                              attributes => [
    ...
    if($self->get_ipAddress && blessed $self->get_ipAddress && $self->get_ipAddress->can("getDOM")) {
        my $ipAddressDOM = $self->get_ipAddress->getDOM($port);
        $ipAddressDOM?$port->appendChild($ipAddressDOM):$self->get_LOGGER->logdie("Failed to append  ipAddress element with value:" .  $ipAddressDOM->toString);
    }
        ...
        if($self->{$subname} && (ref($self->{$subname}) eq 'ARRAY' ||  blessed $self->{$subname})) {
            ...
            foreach my $el (@array) {
                if(blessed $el && $el->can('querySQL'))  {
                    $el->querySQL($query);
                    $self->get_LOGGER->debug("Querying port  for subclass $subname");
                } else {
                    $self->get_LOGGER->logdie("Failed for port Unblessed member or querySQL is not implemented by subclass $subname");
                }
           }
        }
    }
...
=head2  asString()
 ...
 returns nicely formatted XML string  representation of the  port object
...
sub fromDOM {
	...
	my $element;
        ...
        if ($tagname eq  'ipAddress' && $nsid eq 'nmtl3' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::PINGERTOPO_DATATYPES::v2_0::nmtl3::Topology::Domain::Node::Port::IpAddress->new($childnode)
                };
