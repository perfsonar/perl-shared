...
Conf.pm - Library to read configuration files.

=head1 DESCRIPTION

This module is used to set configuration parameters for the OWP one-way-ping
mesh configuration.  To add additional "scalar" parameters, just start using
them. If the new parameter is a BOOL then also add it to the BOOL hash here. If
the new parameter is an array then add it to the ARRS hash.
