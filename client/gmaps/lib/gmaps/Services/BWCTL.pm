...
=cut
package gmaps::Services::BWCTL;
use base 'gmaps::Service';

our $logger = Log::Log4perl->get_logger( "gmaps::Services::BWCTL");
...
sub new
{
	my gmaps::Services::BWCTL $class = shift;
...
sub parseData
{
    ...
    $logger->debug( "Getting bwctl data" );
    my $tuples = {};
		...
		if ( $child->localname() eq 'data' )
		{
            ...
            foreach my $datum ( $child->childNodes() ) {
                ...
                if( $datum->localname() eq 'datum' ) {
                    ...
                    my $time = undef;
                    if ( $timeType eq 'iso' ) {
                        $time = $datum->getAttribute( 'timeValue');
                        # parse into epoch
                        $time = Date::Parse::str2time( $time );
                    } elsif ( ! defined $timeType ) {
                        next;
                    } else {
                    ...
                    # get the value
                    my $value = $datum->getAttribute( 'throughput' );
                    # convert to decimal
                    if ( $value =~ m/^(\d+\.\d+)e\+(\d+)$/ ) {
                        my $power = 1 . 0 x $2; 
                        $value = $1 * $power;
#                        $logger->debug( $value . ' -> ' . $1 . ', ' . $2 . ', ' . $value2 );
                    }
                
                    #$logger->debug( "$time : $value");
	                $tuples->{$time}->{out} = $value;
