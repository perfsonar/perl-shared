package perfSONAR_PS::RegularTesting::PSchedulerAddress;
...
override 'parse' => sub {
    ...
    unless (ref($description) and ref($description) eq "HASH") {
        $description = { pscheduler_address => $description };
    }
...
override 'unparse' => sub {
    ...
    if ($result->{pscheduler_address} and scalar(keys %$result) == 1) {
        $result = $result->{pscheduler_address};
    }
