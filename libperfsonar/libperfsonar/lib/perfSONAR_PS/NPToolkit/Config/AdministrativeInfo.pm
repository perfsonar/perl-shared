...
use fields 'SITE_INFO_FILE', 'ADMINISTRATOR_NAME', 'ADMINISTRATOR_EMAIL', 'ORGANIZATION_NAME', 'LOCATION', 'KEYWORDS', 'CITY', 'REGION', 'COUNTRY', 'ZIP_CODE','LATITUDE','LONGITUDE', 'ROLE', 'ACCESS_POLICY', 'ACCESS_POLICY_NOTES', 'SITE_NAME', 'DOMAIN';
...
sub has_admin_info{
    ...
    if ($self->{ADMINISTRATOR_NAME} and $self->{ORGANIZATION_NAME} and $self->{ADMINISTRATOR_EMAIL}) {
        return 1;
    }
    else {
        return 0;
...
sub generate_administrative_info_file {
    ...
    $output .= "organization_name=" . $self->{ORGANIZATION_NAME} . "\n";
